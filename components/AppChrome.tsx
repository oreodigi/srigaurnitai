...
function mergeNav(configured:any[]){const seen=new Set<string>();const out:any[]=[];[...configured,...required].forEach(item=>{if(item?.href&&!seen.has(item.href)){seen.add(item.href);out.push(item)}});return out}
...
export function AppChrome({children}:{children:React.ReactNode}){
 ...
 const desktop=mergeNav(configured);
 ...
 return <div className={`app-shell ui-header-${s.header_style||"standard"} ui-mobile-${s.mobile_header_style||"compact"}`}>
  <header className="topbar"><BrandLogo/><nav className="desktop-nav public-desktop-nav" aria-label="Website navigation">{desktop.map((item:any)=>{const Icon=iconFor(item.href);return <Link href={item.href} key={item.href} className={active(item.href)?"active":""}><Icon size={15}/><span>{item.label}</span></Link>})}</nav><div className="top-actions">{s.show_header_videos!==false&&<Link className="icon-action desktop-utility-video" href="/videos" aria-label="Public videos"><PlayCircle size={21}/></Link>}{s.show_header_notifications!==false&&<Link className="icon-action" href="/account#notifications" aria-label="Notifications"><Bell size={21}/><span className="notification-dot"/></Link>}{s.show_header_account!==false&&<Link className="avatar-action" href="/account" aria-label="My account"><CircleUserRound size={23}/></Link>}<button className="public-mobile-menu-button" type="button" aria-label="Open website menu" aria-expanded={menuOpen} onClick={()=>setMenuOpen(true)}><Menu size={23}/></button></div></header>
  {menuOpen&&<><button className="public-menu-backdrop" aria-label="Close menu" onClick={()=>setMenuOpen(false)}/><aside className="public-mobile-drawer" aria-label="Website menu"><div className="public-drawer-head"><BrandLogo/><button onClick={()=>setMenuOpen(false)} aria-label="Close menu"><X size={22}/></button></div><div className="public-drawer-intro"><small>SRI GAUR NITAI</small><strong>Explore the community</strong><span>Contests, devotional media, events, businesses and spiritual reading.</span></div><nav>
