...
export default async function ContestsPage({searchParams}:{searchParams:Promise<{category?:string;view?:string}>}){
 ...
 return <div className="page">
  ...
  {featured&&<Link className="featured-business-hero" href={`/contests/${featured.slug}`} style={{backgroundImage:`linear-gradient(90deg,rgba(71,4,32,.97),rgba(71,4,32,.58),rgba(71,4,32,.08)),url(${images[featuredCategory?.name]||images.Singing})`}}><div><span className="gold-badge"><Sparkles size={13}/> Featured Contest</span><h2>{featured.title}</h2><p>{featured.description}</p><div className="profile-meta-strip"><span><CalendarDays size={13}/>Closes {featured.submission_close_at?new Date(featured.submission_close_at).toLocaleDateString("en-IN",{day:"numeric",month:"short"}):"Open"}</span><span><IndianRupee size={13}/>{Number(featured.participation_fee)?`₹${Number(featured.participation_fee).toLocaleString("en-IN")}`:"Free entry"}</span><span><Trophy size={13}/>₹{Number(featured.first_prize||0).toLocaleString("en-IN")} first prize</span></div><span className="btn gold-btn">View & Participate <ChevronRight size={15}/></span></div></Link>}
  <section className="section"><div className="section-head"><div><h2>All Active Contests</h2><p>Open any card to see rules, prizes and recent public entries.</p></div></div><div className="business-grid">{contests.map((c:any)=>{const cat=categoryOf(c);const closes=c.submission_close_at?new Date(c.submission_close_at).toLocaleDateString("en-IN",{day:"numeric",month:"short",year:"numeric"}):"Open";const img=images[cat?.name]||images.Singing;return <Link className="business-card" href={`/contests/${c.slug}`} key={c.id}><div className="business-thumb" style={{backgroundImage:`url(${img})`}}>{c.is_featured&&<span>Featured</span>}</div><div className="business-card-copy"><div className="business-rating"><Trophy size={13}/> ₹{Number(c.first_prize||0).toLocaleString("en-IN")}</div><h3>{c.title}</h3><p>{cat?.name||"Contest"}</p><span className="business-location"><CalendarDays size={13}/>{closes}</span><div className="service-mini"><em>{Number(c.participation_fee)?`Entry ₹${Number(c.participation_fee).toLocaleString("en-IN")}`:"Free Entry"}</em><em>Video Contest</em></div><span className="btn btn-outline" style={{marginTop:10}}>View Contest</span></div></Link>})}</div></section>
