...
export default async function HomePage(){
 const [{data:contests},{data:businesses},{data:videos},{data:campaigns}]=await Promise.all([
  supabase.from("contests").select("id,title,slug,description,banner_url,cta_label,theme_start,theme_end,homepage_order,is_featured,contest_categories(name)").eq("is_active",true).eq("show_on_homepage",true).order("homepage_order").order("is_featured",{ascending:false}).limit(8),
  supabase.from("businesses").select("id,name,slug,city,state,description,photos,business_categories(name)").eq("status","approved").order("is_featured",{ascending:false}).limit(4),
 ...
 <HomeHero slides={(campaigns||[]) as any[]}/>
 ...
 <section className="section"><div className="section-head"><div><h2><Trophy size={19}/> Current Contests</h2></div><Link className="section-link" href="/contests">View all</Link></div><div className="contest-rail">{(contests||[]).map((c:any)=><Link className="mini-contest" href={`/contests/${c.slug}`} key={c.id} style={c.banner_url?{backgroundImage:`linear-gradient(120deg,${c.theme_start||'#6f0a31'}ee,${c.theme_end||'#c52b45'}cc),url(${c.banner_url})`,backgroundSize:'cover',backgroundPosition:'center'}:{}}><h3>{c.title}</h3><p>{c.description}</p><span className="hero-cta">{c.cta_label||'Participate Now'}</span><span className="emoji">🏆</span></Link>)}</div></section>
