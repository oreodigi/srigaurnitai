...
export default async function EventsPage(){
 ...
 return <div className="page">
  ...
  <section className="featured-business-hero" style={{backgroundImage:"linear-gradient(90deg,rgba(82,4,37,.96),rgba(82,4,37,.58),rgba(82,4,37,.08)),url(https://images.unsplash.com/photo-1519741497674-611481863552?auto=format&fit=crop&w=1500&q=85)"}}><div><span className="gold-badge"><Sparkles size={13}/> Free Event Video Services</span><h2>Make Your Special Moments Divine</h2><p>Submit your celebration video, choose a publishing package, request a preferred date and track it through review, scheduling and publishing.</p><Link className="btn gold-btn" href="/events/submit">Publish Your Video Free</Link></div></section>

  <section className="section"><div className="section-head"><div><h2>Event Categories</h2><p>All event publishing services are currently free.</p></div></div><div className="business-grid">{(categories||[]).map((c:any)=>{const v=eventVisuals[c.name];const Icon=v?.icon||Sparkles;const img=c.image_url||v?.img||"https://images.unsplash.com/photo-1492684223066-81342ee5ff30?auto=format&fit=crop&w=900&q=82";return <Link className="business-card" href={`/events/${c.slug}`} key={c.id}><div className="business-thumb" style={{backgroundImage:`url(${img})`}}><span>{Number(c.base_price)>0?`₹${Number(c.base_price).toLocaleString("en-IN")}`:"FREE"}</span></div><div className="business-card-copy"><div className="business-rating"><Icon size={13}/> Event Service</div><h3>{c.name}</h3><p>{c.description||"Video review, publishing-date request and status tracking."}</p><span className="btn btn-outline" style={{marginTop:10}}>View Event Service</span></div></Link>})}</div></section>

  <section className="section"><div className="section-head"><div><h2><Crown size={18}/> Publishing Packages</h2><p>All active packages are currently free.</p></div></div><div className="package-grid">{(packages||[]).map((p:any,i:number)=><Link className={i===1?"package-card featured":"package-card"} href={`/events/submit?package=${p.id}`} key={p.id}>{i===1&&<span className="gold-badge">Most Popular</span>}<h3>{p.name}</h3><b>{Number(p.price_modifier)>0?`+ ₹${Number(p.price_modifier).toLocaleString("en-IN")}`:"FREE"}</b><p>{p.description}</p><ul>{(p.features||[]).map((f:string)=><li key={f}>✓ {f}</li>)}</ul><span className="btn btn-primary">Choose Free Package</span></Link>)}</div></section>
  ...
  <section className="section pretty-panel"><h2><UsersRound size={20}/> Manage Your Requests</h2><p>Signed-in users can track requested publishing dates, approval/rejection, scheduled status and final published links from My Account.</p><div style={{display:"flex",gap:8,flexWrap:"wrap"}}><Link className="btn btn-primary" href="/account#events">My Event Requests</Link><Link className="btn btn-gold" href="/events/submit">Create Free Request</Link></div></section>
 </div>;
}
