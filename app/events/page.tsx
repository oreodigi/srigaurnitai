...
export default async function EventsPage(){
 ...
 return <div className="page">
  ...
  <section className="section"><div className="section-head"><div><h2>Event Categories</h2><p>Choose the occasion that best matches your request.</p></div></div><div className="business-grid">{(categories||[]).map((c:any,i:number)=>{const v=eventVisuals[c.name];const Icon=v?.icon||Sparkles;const img=v?.img||"https://images.unsplash.com/photo-1492684223066-81342ee5ff30?auto=format&fit=crop&w=900&q=82";return <Link className="business-card" href={`/events/submit?category=${c.slug}`} key={c.id}><div className="business-thumb" style={{backgroundImage:`url(${img})`}}><span>₹{Number(c.base_price).toLocaleString("en-IN")}</span></div><div className="business-card-copy"><div className="business-rating"><Icon size={13}/> Event Service</div><h3>{c.name}</h3><p>Video review, publishing-date request and status tracking.</p><span className="btn btn-outline" style={{marginTop:10}}>Select Occasion</span></div></Link>})}</div></section>
