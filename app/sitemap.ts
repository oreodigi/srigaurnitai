...
export default async function sitemap():Promise<MetadataRoute.Sitemap>{
 ...
 return [
  ...
  ...(events||[]).filter((x:any)=>!x.seo_noindex).map((x:any)=>({url:`${base}/events/${x.slug}`,lastModified:x.updated_at?new Date(x.updated_at):undefined,changeFrequency:"monthly" as const,priority:0.7})),
